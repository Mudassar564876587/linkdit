...
export async function createSubmission(formData: FormData) {
  ...
  if (Array.isArray(raw.galleryFiles)) {
    for (const f of raw.galleryFiles) {
      if (f instanceof File) galleryUrls.push(await uploadStorage("tool-galleries", f, user.id))
    }
  }
...
async function uploadStorage(bucket: string, file: File, userId: string): Promise<string> {
  const supabase = await createServerSupabaseClient()
